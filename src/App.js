...
const App = () => {
  ...
  const onEditTask = (listId, taskObj) => {
    ...
    axios.patch('http://localhost:3003/tasks/' + taskObj.id,{text:newTaskText});
  }
  ...
  useEffect(() => {
    axios.get('http://localhost:3003/lists?_expand=color&_embed=tasks').then(response => {
      ...
      <div className="todo__tasks">
        <Route exact path="/">
          {lists && lists.map((list, index) => (
            <Tasks key={index} list={list} onAddTask={onAddTask} onEditTitle={onEditListTitle} withoutEmpty />
          ))}
        </Route>
        {lists && activeItem && <Tasks onEditTask={onEditTask} onRemoveTask={onRemoveTask} list={activeItem} onAddTask={onAddTask} onEditTitle={onEditListTitle} />}
      </div>
    </div>
  )
}
